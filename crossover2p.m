...
function[kinder]=crossover2p(popSize,childNum,chromMatrix,chromLen)
popMag = popSize;
cnt=0;                                  % counter initialisation
chrlim1=chromLen-1;                     % chromosome limit 1
chrlim2=chromLen-2;                     % chromosome limit 2
...
for jin=1:round(childNum/2),            % run for half the number of children, since each iteration creates two
	...
	if pt1==chrlim1                     % if first point is one less than the chromosome length...
		pt2=chromLen;                   % ... the second point will be at the end of the chromosome
	else			
		pt2=round((chromLen-pt1)*rand(1))+pt1;	% if not then the second point is somewhere above the first
	end
	...
	parent1(1,:)=chromMatrix(cnt1,:);			% first parent selected 
	parent2(1,:)=chromMatrix(cnt2,:);			% second parent selected
