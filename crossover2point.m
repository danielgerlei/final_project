...
function[kinder]=crossover2point(popsize,childNum,chrom,chrlen)
popmag = popsize;
...
for jin=1:round(childNum/2),            % run for half the number of children, since each iteration creates two
    ...
    PartSwap=ceil(3*rand(1));
    Parts=[1 pt1 pt2 chrlen];   
	kin=Parts(PartSwap):Parts(PartSwap);                        % range of section to be exchanged 
