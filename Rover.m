...
classdef Rover < handle
    ...
    methods
        ...
        function[]=getStep(this,maxSpeed,step,roverPointer)
            global NoObstacles;                             
            global XObstacle;                       
            global YObstacle;
            ...
            Xvelocity = this.Xvel;
            Yvelocity = this.Yvel;
            Ka = 1000;          % attraction constant
            Kr = -1500000;      % repulsion constant
        ...
        function[]=exchangeElite(this,rovers,neighbours)
            ...
            for n=1:size(neighbours,2)
                 for e=1:elitePop
                     roverF(e)= getStrength(rovers(n).decodedChrom(1,e),rovers(n).decodedChrom(2,e));
                     thisF(e)= getStrength(this.decodedChrom(1,e),this.decodedChrom(2,e));
                 end
                 for e=1:elitePop
                     if(roverF(e)<thisF(e))
                        this.decodedChrom(:,e) = rovers(n).decodedChrom(:,e);
                        this.chromosomes(e,:) = rovers(n).chromosomes(e,:);
                     end
                     if(roverF(e)>thisF(e))
                        rovers(n).decodedChrom(:,e) = this.decodedChrom(:,e);
                        rovers(n).chromosomes(e,:) = this.chromosomes(e,:);
                     end
                 end
            end
        end
